...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    child_window->DC.CursorPos += ImVec2(10, 30);
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip))
            {
                ...
                draw->AddRectFilled(ImVec2(wp.x, wp.y), ImVec2(wp.x + iMainTextSize.x + 40, wp.y + ws.y), ImColor(30, 30, 36, alpha));
                draw->AddText(ImVec2(wp.x + 20, wp.y + 30 - iMainTextSize.y / 2), ImColor(200, 200, 200, alpha), g_Menu->GetMenuName());
                draw->AddRectFilled(ImVec2(wp.x, wp.y + 60), ImVec2(wp.x + iMainTextSize.x + 40, wp.y + 61), ImColor(45, 47, 54, alpha));
                draw->AddRectFilled(ImVec2(wp.x, wp.y + ws.y - 55), ImVec2(wp.x + iMainTextSize.x + 40, wp.y + ws.y - 54), ImColor(45, 47, 54, alpha));

                PopFont();

                draw->AddRectFilled(ImVec2(wp.x + iMainTextSize.x + 40, wp.y), ImVec2(wp.x + iMainTextSize.x + 41, wp.y + ws.y), ImColor(45, 47, 54, alpha));

                AddCircleImageFilled(g_Menu->GetTexture( ), ImVec2(wp.x + 24, wp.y + ws.y - 27), 20, ImColor(255, 255, 255, alpha), 360);

                PushFont(ImGui::GetIO().Fonts->Fonts[5]);
                draw->AddText(ImVec2(wp.x + 50, wp.y + ws.y - 47), ImColor(200, 200, 200, alpha), "Administrator");
                draw->AddText(ImVec2(wp.x + 50, wp.y + ws.y - 27), ImColor(200, 200, 200, alpha), "Till: 01.14.2038");
                PopFont();

                outline(window, 0, ImColor(10, 12, 14));
                outline(window, 1, ImColor(45, 47, 54));
                //outline(window, 2, ImColor(45, 47, 54, 150));
            }
            else 
            {
                ...
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(11, 13, 18), 0, ImDrawCornerFlags_Bot);
                    draw->AddRect(wp, wp + ws, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
                    draw->AddRect(wp + ImVec2(1, 1), wp + ws - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha), 0, ImDrawCornerFlags_All);
                }
            }
