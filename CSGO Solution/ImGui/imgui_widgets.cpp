...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, ImVec2(check_bb.Min.x + it_pos_active->second, check_bb.Max.y), ImColor(15, 17, 19, (int)(g.Style.Alpha * 255)), 10);
        ...
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(it_pos_active->second, 6), it_size_active->second, ImColor(49, 156, 255, (int)(g.Style.Alpha * 255)), 16);
    }
    else
    {
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(15, 17, 19, (int)(g.Style.Alpha * 255)));
    window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y - 1), ImColor(10, 12, 14, (int)(g.Style.Alpha * 255)));
    window->DrawList->AddRect(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(45, 47, 54, (int)(g.Style.Alpha * 255)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
   ...
   if (it_size == size.end())
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(12, 14, 18, (int)(g.Style.Alpha * 255)), 2);

    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), ImColor(34, 149, 255, (int)(g.Style.Alpha * 255)), 2);

    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y - 3), it_size->second, ImColor(34, 149, 255, (int)(g.Style.Alpha * 255)), 12);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(size_arg, 240, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    if (!is_multiline)
    {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 17, 19, (int)(g.Style.Alpha * 255)));
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(10 / 255.f, 12 / 255.f, 14 / 255.f, g.Style.Alpha));
        window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), ImColor(45 / 255.f, 47 / 255.f, 54 / 255.f, g.Style.Alpha));
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 hover_dis = ImVec4(49 / 255.f, 156 / 255.f, 255 / 255.f, 1.f);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
