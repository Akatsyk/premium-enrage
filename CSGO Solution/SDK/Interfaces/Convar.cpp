...
public:
    virtual bool RegisterConCommandBase(ConCommandBase *pVar)
    {
        ...
        g_Globals.m_Interfaces.m_CVar->RegisterConCommand(pVar);
        return true;
    }
};
...
{
    if(!g_Globals.m_Interfaces.m_CVar || s_bRegistered)
        return;

    assert(s_nDLLIdentifier < 0);
    s_bRegistered = true;
    s_nCVarFlag = nCVarFlag;
    s_nDLLIdentifier = g_Globals.m_Interfaces.m_CVar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
    ...
    assert(s_nDLLIdentifier >= 0);
    g_Globals.m_Interfaces.m_CVar->UnregisterConCommands(s_nDLLIdentifier);
    s_nDLLIdentifier = -1;
    s_bRegistered = false;
}
...
{
    if( g_Globals.m_Interfaces.m_CVar ) 
        g_Globals.m_Interfaces.m_CVar->UnregisterConCommand( this );
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
    ...
    if( g_Globals.m_Interfaces.m_CVar )
        g_Globals.m_Interfaces.m_CVar->CallGlobalChangeCallbacks( this, pszOldValue, flOldValue );
}
